...
#function to generate citation for package
citation("tidyLPA")


#figure out model fit; testing model type & profile number
##testing out 1-4 profiles with models 1,2,3,6 (models 4 & 5 aren't typically used/available with tidyLPA )
##model specifications: 1) equal variances, covariances = 0; 2) varying variances, covariances = 0; 3) equal all; 6) varying all
##warnings: no convergence for  model 2 & model 6 

set.seed(10101010)
LCAestimations <- estimate_profiles(df = longData[,6:12], n_profiles = 1:10, models = 1) 

LCAPlotData <- get_fit(LCAestimations) %>%
  select(Classes, AIC, BIC) %>%
  pivot_longer(cols = c(AIC, BIC), names_to = "Criterion", values_to = "Value")

##elbow plot to determine class number
ggplot(LCAPlotData, aes(x = Classes, y = Value, color = Criterion)) +
  geom_line()+
...
#2 plots
LCAprofiles2 <- longData[,6:12] %>%
  estimate_profiles(2)

plots2 <- LCAprofiles2 %>%
  plot_profiles()

plots2 <- plots2 + facet_wrap(~Class)

#profile data
profileAssignment <- get_data(LCAprofiles2)

#add classification column to original df
longData$profile <- profileAssignment$Class


###Data analysis - 2 profiles###

#save means and SEs for each trait in each class
profileCentroids <- as.data.frame(get_estimates(LCAprofiles2))

profileCentroidsWide <- profileCentroids[,c(1:3, 6)] %>%
...
profileCentroidsWide$sd <- sqrt(profileCentroidsWide$variance)


#gender breakdown by profile
profileGender <-table(longData$gender,longData$profile)

#overall between trait variance for each profile
profile1AvgVar <- var(profileCentroidsWide[1:7,3]) 
profile2AvgVar <- var(profileCentroidsWide[8:14,3]) 


#are people creating ideal partners of the same or different profiles? 

##transfer profile number back to wide df
data$blueProfile<-longData$profile[longData$partner=="idealBlue"]
data$orangeProfile<-longData$profile[longData$partner=="idealOrange"]

##create variable for whether participants' partners have the same or different profiles (same = 1, diff = 0)
data$sameOrDiff<-apply(data[,134:135],1,function(x)
  sum(duplicated(x))) ##checking for duplicates (if same cluster = duplicate = 1; if diff clusters = 0)

##get sameOrDiff as function of gender
genderDiff<-table(data$gender, data$sameOrDiff)
...
###this combo order is arbitrary (numerically smaller cluster always listed first)
###arbitrary-ness is necessary for gender, cluster combo chisq later
data$profileCombo<-apply(data[,134:135],1,function(x) paste0(sort(as.numeric(x)),collapse=","))
comboRates <- table(data$profileCombo)
...
##do preferences for partner orange predict preferences for partner blue?
chisqProfile<-chisq.test(table(data$blueProfile, data$orangeProfile)) 
chisqProfileW <- cohenW(x = chisqProfile$observed, p = chisqProfile$expected)  #effect size

##are men and women are choosing combos of partner orange and blue at diff rates?
chisqGender<-chisq.test(table(data$gender,data$profileCombo))
chisqGenderW <- cohenW(x = chisqGender$observed, p = chisqGender$expected) #effect size

##raw numbers in each cluster combo by gender
clustComboGender<-table(data$blueProfile,data$orangeProfile,data$gender)




##Proportion of participants with at least 1 partner in each profile

###Create blank columns in data
data$profile1 <- NA ###for cluster 1
data$profile2<- NA ###for cluster 2


###for loop to fill columns
for(i in 1:NROW(data)){
  ifelse(data$orangeProfile[i] == 1 | data$blueProfile[i] == 1, data$profile1[i]<-1, data$profile1[i]<-0)
  
}

for(i in 1:NROW(data)){
  ifelse(data$orangeProfile[i] == 2 | data$blueProfile[i] == 2, data$profile2[i]<-1, data$profile2[i]<-0)
  
}

###is one sex more likely to have at least one partner in profile 1?
chisqProfile1 <- chisq.test(table(data$gender, data$profile1))
chisqProfile1W <- cohenW( x = chisqProfile1$observed, p = chisqProfile1$expected) #effSize

###is one sex more likely to have at least one partner in profile 2? 
chisqProfile2 <- chisq.test(table(data$gender, data$profile2))
chisqProfile2W <- cohenW( x = chisqProfile2$observed, p = chisqProfile2$expected) #effect size




#Permuation Analysis: Are ideal partners more similar than chance? 
...
###creating a null distribution by randomly shuffling profile assignments
for(a in 1:100000){
  
  aNull<-sample(data$blueProfile)
  bNull<-sample(data$orangeProfile)
  
  ####Compute sameordiff (whether profile A and profile B are the same or different)
  sodNull<-ifelse(aNull==bNull, 1, 0)
...
#Descriptives: How many ideal partners are in each profile?

##for people who wanted both partners in same profile
sameProfile <- table(data$blueProfile[data$sameOrDiff == 1]) 

##for people who wanted partners in different clusters
diffProfile <- table(data$blueProfile[data$sameOrDiff ==0])  


#Confusion Matrices

##overall profile combo frequency

###create matrix dataframe
overallMatrix <- data.frame(((table(data$blueProfile, data$orangeProfile))/sum(table(data$blueProfile, data$orangeProfile)))*100)

###relabel column names
colnames(overallMatrix) <- c("blueProfile", "orangeProfile", "comboFrequency")
...
###plot matrix
overallMatrixPlot <- ggplot(overallMatrix, aes(x= blueProfile, y = orangeProfile, fill = comboFrequency)) +
  geom_tile(color = "white") +
  geom_text(label = overallMatrix$comboFrequency)+
  scale_fill_gradient(low = "white", high = "#028A0F", na.value = "whitesmoke") +
  scale_x_discrete(labels = c('Good in Bed, \nAttractive, & Kind', 'Well-Rounded')) +
  scale_y_discrete(labels = c('Good in Bed, \nAttractive, & Kind', 'Well-Rounded')) +
  labs(x = "Partner Blue", y = "Partner Orange", fill = "Combination Freq.") +
  theme(text = element_text(size = 13), 
        panel.background = element_rect(fill = "#f5f5f5", color = "black"))

#ggsave("overallMatrix.jpeg", plot=last_plot(), width=150, height=100, units="mm", path ="/Users/ashle/Desktop", scale = 1, dpi=300, limitsize=TRUE)



##Profile combination frequency by gender

###men

####create matrix dataframe
maleMatrix <- data.frame(((table(data$blueProfile[data$gender == 1], data$orangeProfile[data$gender == 1]))/sum(table(data$blueProfile[data$gender == 1], data$orangeProfile[data$gender == 1])))*100)

####relabel column names
colnames(maleMatrix) <- c("blueProfile", "orangeProfile", "comboFrequency")
...
####plot matrix
maleMatrixPlot <- ggplot(maleMatrix, aes(x= blueProfile, y = orangeProfile, fill = comboFrequency)) +
  geom_tile(color = "white") +
  geom_text(label = maleMatrix$comboFrequency)+
  scale_fill_gradient(low = "white", high = "#028A0F", na.value = "whitesmoke") +
  scale_x_discrete(labels = c('Good in Bed, \nAttractive, & Kind', 'Well-Rounded')) +
...
####create matrix dataframe
femaleMatrix <- data.frame(((table(data$blueProfile[data$gender == 0], data$orangeProfile[data$gender == 0]))/sum(table(data$blueProfile[data$gender == 0], data$orangeProfile[data$gender == 0])))*100)

####relabel column names
colnames(femaleMatrix) <- c("blueProfile", "orangeProfile", "comboFrequency")
...
####plot matrix
femaleMatrixPlot <- ggplot(femaleMatrix, aes(x= blueProfile, y = orangeProfile, fill = comboFrequency)) +
  geom_tile(color = "white") +
  geom_text(label = femaleMatrix$comboFrequency)+
  scale_fill_gradient(low = "white", high = "#028A0F", na.value = "whitesmoke") +
  scale_x_discrete(labels = c('Good in Bed, \nAttractive, & Kind', 'Well-Rounded')) +
...
#panel plot of both of these graphs
matrixPanelPlot <- ggarrange(femaleMatrixPlot, maleMatrixPlot, nrow=1, ncol=2, 
...
###LCA with wide data -- 
##profiles are participants and how they allocate across partners 
##(not profiles as ideal partners)

set.seed(10101010)
LCAestimationsWide <- estimate_profiles(df = data[,14:27], n_profiles = 1:10, models = 1) 

LCAPlotDataWide <- get_fit(LCAestimationsWide) %>%
  select(Classes, AIC, BIC) %>%
  pivot_longer(cols = c(AIC, BIC), names_to = "Criterion", values_to = "Value")

##elbow plot to determine class number
ggplot(LCAPlotDataWide, aes(x = Classes, y = Value, color = Criterion)) +
  geom_line()+
  scale_color_manual(values = c("AIC" = "red", "BIC"= "blue")) +
  geom_point()+
...
plots8Wide <- data[,14:27] %>%
  estimate_profiles(8) %>%
  plot_profiles()

plots8Wide +facet_wrap(~Class) +
  theme(
...
plots2Wide <- data[,14:27] %>%
  estimate_profiles(2) %>%
  plot_profiles()

plots2Wide +facet_wrap(~Class) +
  theme(
    axis.text.x = element_text(
      angle = 90, 
      vjust = .5, 
...
plots3Wide <- data[,14:27] %>%
  estimate_profiles(3) %>%
  plot_profiles()

plots3Wide +facet_wrap(~Class) +
  theme(
    axis.text.x = element_text(
      angle = 90, 
      vjust = .5, 
...
plots4Wide <- data[,14:27] %>%
  estimate_profiles(4) %>%
  plot_profiles()

plots4Wide +facet_wrap(~Class) +
  theme(
    axis.text.x = element_text(
      angle = 90, 
      vjust = .5, 
